...
samp1 %>%
  summarise(sample_mean = mean(area), pop_med = median(area),
...
sample_means50 %>%
  summarise(mean_X_bar = mean(x_bar), SE_X_med = median(x_bar),
            se= sd(x_bar), pop_iqr = IQR(x_bar),
            X_bar_min = min(x_bar), X_bar_max = max(x_bar),
            x_bar_q1 = quantile(x_bar, 0.25),  # first quartile, 25th percentile
            x_bar_q3 = quantile(x_bar, 0.75))  # third quartile, 75th percentile
...
sample_mean_price <- ames %>%
                    ...
                    summarise(x_price = mean(price))
sample_mean_price %>%
  summarise(mean_X_bar = mean(x_price), SE_X_med = median(x_price),
            se= sd(x_price), pop_iqr = IQR(x_price),
            X_bar_min = min(x_price), X_bar_max = max(x_price),
            x_bar_q1 = quantile(x_price, 0.25),  # first quartile, 25th percentile
            x_bar_q3 = quantile(x_price, 0.75))  # third quartile, 75th percentile
```
