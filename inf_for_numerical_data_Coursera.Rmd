...
<li> Both distributions are extremely right skewed. yes</li>
...
<li> yes $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$ </li>
...
<li> yes We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers. </li>
</ol>
```{r habit-weight-ci}
...
inference(y = weeks, data = nc, statistic = "mean", type = "ci", null = 0, conf_level = .99,
          alternative = "twosided", method = "theoretical")
```
...
inference(y = mage, x = mature, data = nc, statistic = "mean", type = "ci", null = 0, 
          alternative = "twosided", method = "theoretical")
```
```{r}
summary(nc$mage)
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
#do white mothers have heavier babies than non-white mothers?
...
```
```{r}
inference(y = weight, x = whitemom, data = nc, statistic = "mean", type = "ci", null = 0, 
